...
const ROLE_RULES = [
  ...
  ['waiting',      ['waiting', 'waiter', 'waitress', 'server', 'front of house', ' foh ', 'runner', 'wait staff', 'floor staff', 'floor attendant', 'floor team']],
...
const HOTEL_BRANDS = ['maldron', 'clayton', 'kimpton', 'hilton', 'marriott', 'radisson', 'ibis', 'novotel',
  'mercure', 'accor', 'jurys', 'malmaison', 'hotel indigo', 'ihg', 'intercontinental', 'holiday inn', 'apex',
  'valor hospitality', 'premier inn', 'whitbread', 'travelodge', 'village hotel', 'dalata', 'doubletree',
  'moxy', 'hampton', 'hoxton', 'scotsman']
...
const VENUE_RULES = [
  ...
  ['qsr',        ['fast food', 'takeaway', 'drive thru', 'drive-thru', 'mcdonald', 'kfc', 'burger king', 'subway', 'nando', 'wingstop', 'five guys', 'popeyes', 'greggs']],
  ...
  ['pub',        ['pub', 'tavern', 'arms', 'brewery', 'alehouse', 'stonegate', 'mitchells & butlers', 'marston', "wetherspoon"]],
...
// ── Industry ────────────────────────────────────────────────────────────────
// Shiftly serves hospitality AND retail, so a listing is partitioned into one of
// them (or 'other' when the signal is genuinely ambiguous, never forced).
//
// The order of evidence matters. A ROLE is the strongest signal, because a chef
// is hospitality and a checkout operator is retail wherever they work. Only when
// the role is generic (manager, supervisor, team member) do we fall back to the
// venue or employer to break the tie, and if nothing is clear we return 'other'
// rather than guess, so neither industry board is polluted.

// Retail-clear roles. These are retail in any setting. Title-based, first match.
const RETAIL_ROLE_RULES = [
  ['sales_assistant', ['sales assistant', 'retail assistant', 'store assistant', 'shop assistant',
                       'sales advisor', 'retail advisor', 'customer advisor', 'sales colleague',
                       'retail colleague', 'store colleague', 'sales associate', 'shop floor']],
  ['cashier',         ['cashier', 'checkout', 'till operator', 'checkout operator']],
  ['stock',           ['stockroom', 'stock assistant', 'replenishment', 'merchandiser',
                       'visual merchandise', 'stock control', 'shelf stack']],
  ['retail_manager',  ['store manager', 'retail manager', 'branch manager', 'shop manager',
                       'department manager', 'concession manager']],
  ['retail_supervisor', ['retail supervisor', 'floor supervisor', 'sales supervisor']],
]

export function classifyRetailRole(title) {
  const t = norm(title)
  for (const [role, needles] of RETAIL_ROLE_RULES) {
    if (needles.some((n) => t.includes(n))) return role
  }
  return null
}

// Titles that belong to NEITHER hospitality nor retail. These leak in through
// aggregator keyword searches (a recruitment agency posts a plumber and a chef
// under the same feed). Matched on the TITLE, before any industry signal, so a
// "Financial Controller at Tesco" is off-topic despite Tesco being a retailer.
// Kept specific to avoid catching real roles ("installation engineer", not bare
// "engineer"; "account manager", not bare "manager").
const OFF_TOPIC = [
  // trades and construction
  'joiner', 'plumber', 'electrician', 'installation engineer', 'maintenance engineer',
  'gas engineer', 'multi skilled', 'multiskilled', 'labourer', 'welder', 'carpenter',
  'bricklayer', 'plasterer', 'roofer', 'scaffolder', 'fitter', 'groundworker',
  // office, finance and corporate
  'financial controller', 'accountant', 'account manager', 'account executive',
  'bookkeeper', 'payroll', 'business development', 'administrator', 'administrative',
  'executive assistant', 'data entry', 'weighbridge', 'estimator', 'quantity surveyor',
  // technology
  'software', 'developer', 'it support', 'digital support', 'digital customer support',
  'data analyst', 'network engineer', 'web developer', 'systems engineer',
  // care, medical and social (front-line care, not catering inside a care home)
  'home manager', 'domestic assistant', 'social worker',
  // qualified health professionals, which appear at optician and pharmacy chains
  // but are not retail shift work
  'optometrist', 'optician', 'pharmacist', 'audiologist', 'dentist', 'dental',
  'hygienist', 'physiotherapist', 'nurse',
  // education, sport and other sectors
  'tennis coach', 'football coach', 'sports coach', 'driving instructor', 'estate agent',
]

export const isOffTopic = (title) => {
  const t = norm(title)
  return OFF_TOPIC.some((n) => t.includes(n))
}

// Hospitality role_category values that settle the partition. team_member is
// included because classifyRole only assigns it on HOSPITALITY catch-all titles
// (crew member, f&b, guest service, catering assistant); retail has its own.
// supervisor / management stay OUT: those titles occur in both industries, so
// they fall through to the venue / employer tie-break.
const HOSPITALITY_CLEAR_ROLES = new Set(['kp', 'kitchen', 'barista', 'bar', 'waiting', 'host', 'housekeeping', 'team_member'])

// Well-known UK retailers whose name alone settles an otherwise generic role
// ("Assistant Manager" at Boots is retail). Kept short and high-confidence.
const KNOWN_RETAILERS = [
  'tesco', 'sainsbury', 'asda', 'morrisons', 'aldi', 'lidl', 'co op', 'iceland', 'farmfoods',
  'boots', 'superdrug', 'next', 'primark', 'marks & spencer', 'm&s', 'tk maxx', 'tj maxx',
  'john lewis', 'argos', 'currys', 'b&m', 'home bargains', 'poundland', 'wilko', 'the range',
  'wh smith', 'waterstones', 'card factory', 'pets at home', 'sports direct', 'jd sports',
  'new look', 'river island', 'h&m', 'zara', 'uniqlo', 'ikea', 'screwfix', 'toolstation', 'halfords',
  'holland & barrett', 'savers', 'matalan', 'dunelm', 'wickes', 'b&q', 'homebase',
]

// Word-boundary match, not substring, or "Savers" would match "Specsavers" and
// "Next" would match any word containing it. norm() wraps and single-spaces the
// name, so a space-padded needle only matches whole words.
const isKnownRetailer = (employerName) => {
  const t = norm(employerName)
  return KNOWN_RETAILERS.some((r) => t.includes(` ${r} `))
}

/**
 * hospitality | retail | other.
 *
 * Pass the already-classified roleCategory and venueType where available (the
 * ingest path has them), plus the raw title and employer for the tie-breaks.
 */
export function classifyIndustry({ title, employerName, roleCategory, venueType } = {}) {
  // 0. Off-topic by title, before anything else, so a trade or office role at a
  //    retail/hospitality-adjacent employer is still excluded.
  if (isOffTopic(title)) return 'other'

  const role = roleCategory || classifyRole(title)

  // 1. Unambiguous hospitality role wins outright.
  if (HOSPITALITY_CLEAR_ROLES.has(role)) return 'hospitality'

  // 2. Unambiguous retail role wins outright.
  if (classifyRetailRole(title)) return 'retail'

  // 3. Generic role (manager/supervisor/other): break the tie on the venue, which
  //    today is a hospitality-only vocabulary, then the employer. A STORED venue
  //    of 'other' is recomputed here so improved brand rules apply to old rows.
  // brand:employerName so the brand->venue map (Greene King, Hungry Horse) fires
  // on the employer name, not just a dedicated brand field.
  const venue = venueType && venueType !== 'other' ? venueType : classifyVenue({ brand: employerName, employerName, title })
  if (venue && venue !== 'other') return 'hospitality'
  if (isKnownRetailer(employerName)) return 'retail'

  // 4. No clear signal. Do not force it.
  return 'other'
}
...
// Same, but the currency symbol is optional. Used only when a caller opts in via
// requireCurrency:false, for feeds that write pay without a £ ("14.56 per hour").
// Must START with a digit: without the £ to anchor it, a bare comma in a plain
// [\d,]+ class would match on its own ("Chef, 14.56") and misalign the capture.
const NUM_NO_CCY = String.raw`£?\s*(\d[\d,]*(?:\.\d{1,2})?)`
 ...
 */
export function parsePayFromText(text, { requireCurrency = true } = {}) {
  if (!text) return null
  const s = String(text)

  const numPat = requireCurrency ? NUM : NUM_NO_CCY
  // Without a required £, the single-figure pass is looser, so its cue window is
  // tightened: the cue must sit right after the amount ("14.56 per hour"), which
  // is how pay reads when the symbol is dropped. The plausible() bounds and the
  // specific multi-character cues (never a bare "hour") are what keep stray
  // numbers like "40 hours per week" or "28 days holiday" from being read as pay.
  const win = requireCurrency ? 30 : 14

  // Pass 1, ranges, with the cue allowed to trail the pair.
  const rangeRe = new RegExp(`${numPat}\\s*(?:-|–|—|to)\\s*${numPat}([^.]{0,25})`, 'gi')
  let m
  ...
  // Pass 2, a single figure with its cue. The range pass above has already
  // claimed pairs.
  const oneRe = new RegExp(`${numPat}(.{0,${win}})`, 'gi')
...
const AGENCY_SIGNALS = [
  ...
  // Reed runs a hospitality desk under its own brand, and Goldstar is a chef
  // recruiter. Neither contains a generic agency word.
  'reed hospitality', 'goldstar chefs',
]
