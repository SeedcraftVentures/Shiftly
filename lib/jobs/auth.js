...
// is the spam control that replaced the account gate. The waitlist choice rides
// in the token (w), so the verify step joins the waitlist only if the poster
// actually opted in, and the choice cannot be tampered with in transit.
export const publishToken = (listingId, email, wantsWaitlist = false) =>
  signToken({ p: 'publish', l: listingId, e: email.toLowerCase(), w: wantsWaitlist ? 1 : 0 }, MAGIC_LINK_TTL)
