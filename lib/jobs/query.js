...
import { PER_PAGE, showsPay, MAX_AGE_DAYS, REAL_LIVING_WAGE_HOURLY, REAL_LIVING_WAGE_ANNUAL, meetsLivingWage, citySlug, TARGET_TOWNS } from './taxonomy'
...
// The two industries the board serves. Anything classified outside them is never
// shown, per the product rule "if it is not hospitality or retail it does not
// appear". Kept as a constant so the boards and the toggle share one source.
export const BOARD_INDUSTRIES = ['hospitality', 'retail']

function applyFilters(qb, f) {
  qb = qb.eq('status', 'live').gte('posted_at', cutoffISO())
  // Industry gate. A single industry when a board or the toggle asks for one,
  // otherwise both, and never 'other'.
  const industries = f.industry && BOARD_INDUSTRIES.includes(f.industry) ? [f.industry] : BOARD_INDUSTRIES
  qb = qb.in('industry', industries)
  ...
  // Real Living Wage: employer-stated pay whose floor clears the rate. Mirrors
  // meetsLivingWage() in the query layer, hourly compared directly and annual
  // against the full-time-week equivalent. Implies pay is shown.
  if (f.livingWage) {
    qb = qb
      .eq('pay_is_estimated', false)
      .or(`and(pay_period.eq.hourly,pay_min.gte.${REAL_LIVING_WAGE_HOURLY}),and(pay_period.eq.annual,pay_min.gte.${REAL_LIVING_WAGE_ANNUAL})`)
  }
  return qb
}
...
export async function getListingForPage(slug) {
  ...
  // Off-topic rows are not part of the board and get no page, live or closed.
  if (!BOARD_INDUSTRIES.includes(data.industry)) return null
...
/**
 * Distinct cities across the live board (both industries), each with its slug and
 * live count, most first. Feeds the town pages and any "browse by town" list.
 */
export async function listCities() {
  ...
  const { data } = await supabaseAdmin
    ...
    .eq('status', 'live')
    .gte('posted_at', cutoffISO())
    .in('industry', BOARD_INDUSTRIES)
    .not('city', 'is', null)
    .limit(5000)
  const counts = {}
  for (const r of data || []) if (r.city) counts[r.city] = (counts[r.city] || 0) + 1
  return Object.entries(counts)
    .map(([city, count]) => ({ city, slug: citySlug(city), count }))
    .sort((a, b) => b.count - a.count)
}

/** Resolve a town-page slug back to the stored city value, or null. */
export async function resolveCity(slug) {
  if (!slug) return null
  const wanted = String(slug).toLowerCase()
  const match = (await listCities()).find((c) => c.slug === wanted)
  return match ? match.city : null
}

/**
 * Resolve a /jobs/in/[town] slug to a town, whether or not it has jobs yet.
 * Returns { name, hasJobs } or null. A live city wins (its exact stored name); a
 * seeded target town renders even with no jobs; anything else is null (404).
 */
export async function resolveTown(slug) {
  const city = await resolveCity(slug)
  if (city) return { name: city, hasJobs: true }
  const target = TARGET_TOWNS.find((t) => t.slug === String(slug || '').toLowerCase())
  return target ? { name: target.name, hasJobs: false } : null
}
...
export async function getFacets(f = {}) {
  ...
  let qb = supabaseAdmin
    .from('Job Listings')
    .select('role_category,venue_type,city,contract_type,employer_name,pay_min,pay_max,pay_period,pay_is_estimated,posted_at')
  ...
  return {
    ...
    stats: {
      ...
      livingWage: rows.filter(meetsLivingWage).length,
