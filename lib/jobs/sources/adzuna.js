...
import {
  ...
  isAgency, classifyIndustry, tidyCity, parsePayFromText, buildSlug,
} from '../classify.js'
...
export const RETAIL_CATEGORY = 'retail-jobs'
...
function toRow(r, country) {
  ...
  const title = r.title ? String(r.title).replace(/<\/?[^>]+>/g, '').trim() : 'Untitled role'
  return {
    source: 'adzuna',
...
/**
 * Fetch listings around a region's anchor towns from one Adzuna category. The
 * category is a fetch hint only: classifyIndustry re-derives each row's industry
 * from its title and employer, so a restaurant manager returned by the retail
 * category is still tagged hospitality, and off-topic rows are dropped.
 * @param {object} opts
 * @param {string} [opts.region='central_scotland']
 * @param {number} [opts.limit=100]     total rows across all anchors
 * @param {string} [opts.category]      Adzuna category tag; hospitality default
 * @param {string} [opts.country='gb']
 * @param {number} [opts.maxDaysOld=30]
 */
export async function fetchRegion({ region = 'central_scotland', limit = 100, category = HOSPITALITY_CATEGORY, country = 'gb', maxDaysOld = 30 } = {}) {
  const c = creds()
  ...
  outer: for (let page = 1; page <= 2; page++) {
    for (const where of cfg.anchors) {
      ...
      for (const r of results) {
        ...
        // Only hospitality and retail belong on the board. classifyIndustry drops
        // the trades, office, care and other off-topic roles Adzuna's category
        // leaks, and tags the rest so the two industry boards can split them.
        if (row.industry === 'other') continue
        rows.push(row)
      }
    }
  }
