...
import {
  ...
  isAgency, isHospitality, tidyCity, buildSlug, parsePayFromText,
} from '../classify.js'
...
export async function fetchRegion({ region = 'central_scotland', limit = 100, keywords = HOSPITALITY_KEYWORDS } = {}) {
  ...
  for (const keyword of keywords) {
    for (const where of cfg.anchors) {
      ...
      for (const r of results) {
        ...
        seen.add(id)
        // Filter on the cheap search fields before spending a detail call.
        if (!isHospitality({ title: r.jobTitle, employerName: r.employerName })) continue
        candidates.push(id)
      }
    }
  }
  ...
  const rows = details
    .filter(Boolean)
    .map(toRow)
    // Re-check against the full record: the detail city/title can refine what the
    // search row implied, and a detail fetch can fail leaving nothing to keep.
    .filter((row) => row.source_id && isHospitality({ title: row.title, employerName: row.employer_name }))
