...
export const TOOLS = [
  ...
  {
    name: 'edit_rota',
    description: 'Make a small change to an already-saved (usually published) rota for one date, without rebuilding it. Use for urgent day-to-day changes: someone off sick (remove), give a shift to someone else (reassign), swap two people (swap), or add a shift for someone (add). It finds the rota for that date automatically. This keeps the live rota and payroll accurate. Get staff_id and shift_id from read_rota or read_workspace. Only do this when the manager clearly asked, and say plainly what changed.',
    input_schema: {
      type: 'object',
      properties: {
        work_date: { type: 'string', description: 'the exact date to change, YYYY-MM-DD' },
        op: { type: 'string', enum: ['remove', 'reassign', 'swap', 'add'] },
        staff_id: { type: 'string', description: 'the person: who to take off (remove), who to add (add), or the FROM person (reassign)' },
        to_staff_id: { type: 'string', description: 'reassign only: the person who takes the shift' },
        staff_a: { type: 'string', description: 'swap only: first person' },
        staff_b: { type: 'string', description: 'swap only: second person' },
        shift_id: { type: 'string', description: 'optional: narrow to one shift if the person has several that day, or the shift pattern to add' },
        start: { type: 'number', description: 'add a custom shift: start in 24h decimal hours (9 = 9am)' },
        end: { type: 'number', description: 'add a custom shift: end in 24h decimal hours (17.5 = 5:30pm)' },
        name: { type: 'string', description: 'add a custom shift: a short label' },
      },
      required: ['work_date', 'op'],
    },
  },
]
...
export async function executeTool(name, input, { origin, cookie }) {
  ...
  try {
    ...
    if (name === 'read_rota') {
      ...
      const shifts = (data?.assignments || []).map((a) => ({ date: a.work_date, day: DAYS[a.day] ?? a.day, staff: a.staff_name, staff_id: a.staff_id, shift: a.shift_name, shift_id: a.shift_id, start: a.start_time, end: a.end_time }))
    ...
    if (name === 'edit_rota') {
      const decToHHMM = (v) => { const h = Math.floor(num(v)); const m = Math.round((num(v) - h) * 60); return `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}` }
      const payload = { work_date: input.work_date, op: input.op }
      for (const k of ['staff_id', 'to_staff_id', 'staff_a', 'staff_b', 'shift_id', 'name']) if (input[k] !== undefined) payload[k] = input[k]
      if (input.start !== undefined) payload.start_time = decToHHMM(input.start)
      if (input.end !== undefined) payload.end_time = decToHHMM(input.end)
      const r = await api('/api/rotas/edit', { method: 'POST', body: JSON.stringify(payload) })
      const d = await r.json().catch(() => null)
      if (!r.ok) return { ok: false, error: d?.error || `Could not change the rota (${r.status}).` }
      return { ok: true, status: d?.status, summary: d?.summary || 'Updated the rota.' }
    }
