...
// ════════════════════════════════════════════════════════════════════════════
//  RulesPanel - universal scheduling rules (per location). Autosaves on change.
//  Rendered full-page on /dashboard/rules and, compactly, as a panel inside the
//  Rota Builder so the whole rota can be shaped on one page.
// ════════════════════════════════════════════════════════════════════════════
...
const RULES = [
  ...
  { key: 'max_consecutive_days', type: 'stepper', label: 'Maximum consecutive days', desc: 'The most days in a row a person can work, counted across weeks too.', min: 1, max: 7, unit: 'd' },
  { key: 'fair_distribution', type: 'toggle', label: 'Fair distribution', desc: 'Spread shifts as evenly as possible across everyone happy to vary their shifts.' },
...
const DEFAULTS = { min_rest_hours: 11, max_consecutive_days: 5, fair_distribution: true, prefer_consecutive_days_off: true }

export function SaveStatus({ T, state }) {
...
export default function RulesPanel({ compact = false }) {
  const { T } = useTheme()
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (Array.isArray(data) && data[0]) { const r = { ...DEFAULTS, ...(data[0].rules || {}) }; setRules(r); rulesRef.current = r; locRef.current = data[0].location_id }
  ...
  if (loading) return <div style={{ fontFamily: T.font, padding: compact ? 20 : 60, textAlign: 'center', color: T.faint }}>Loading rules…</div>
  ...
  const list = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 8 : 12 }}>
      {RULES.map((rule) => {
        ...
        return (
          <Card key={rule.key} pad={compact ? '13px 15px' : '18px 20px'} style={{ display: 'flex', alignItems: 'center', gap: compact ? 14 : 18 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: compact ? 13.5 : 15, fontWeight: 600, color: T.ink, marginBottom: 3, letterSpacing: '-0.01em' }}>{rule.label}</div>
              <p style={{ fontSize: compact ? 12 : 13, color: T.muted, margin: 0, lineHeight: 1.45, letterSpacing: '-0.01em' }}>{rule.desc}</p>
            </div>
            {rule.type === 'stepper'
  ...
  // Compact: just the list, header/status handled by the host (the builder panel).
  if (compact) return <div style={{ fontFamily: T.font, color: T.body }}>{list}</div>
  ...
  return (
    <div style={{ fontFamily: T.font, color: T.body, maxWidth: 780, margin: '0 auto', padding: '40px 32px 64px' }}>
      ...
      <p style={{ fontSize: 13, color: T.faint, margin: '10px 0 24px', lineHeight: 1.5, maxWidth: 620 }}>Coverage, a keyholder on every open and close, contracted hours and anyone on fixed weekly shifts are always protected first. These are the softer preferences the rota balances after that, and it always generates. Changes save automatically.</p>
      {list}
    </div>
  )
}
