    ...
    <nav className="fixed top-5 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-1.5rem)] max-w-5xl">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/waitlist"
            onClick={closeMenus}
            className="px-4 sm:px-5 py-2.5 text-white rounded-full font-semibold transition-all text-sm bg-pink-500 shadow-[0_4px_14px_rgba(255,31,125,0.3)] hover:shadow-[0_6px_20px_rgba(255,31,125,0.4)] hover:-translate-y-0.5 whitespace-nowrap"
          >
            Join Waitlist
          </Link>
        </div>
      ...
      {mobileMenuOpen && (
        <div className={`md:hidden mt-2 rounded-2xl p-3 space-y-1 ${GLASS}`}>
          ...
          <Link href="/waitlist" onClick={closeMenus} className="block w-full text-left px-4 py-2.5 font-semibold rounded-xl text-white bg-pink-500">Join Waitlist</Link>
        </div>
      )}
    </nav>
  )
