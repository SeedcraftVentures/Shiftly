...
const ghostBtn = { fontFamily: 'inherit', fontSize: 13, fontWeight: 700, color: '#6B7280', background: '#fff', border: '1px solid #E5E7EB', borderRadius: 10, padding: '10px 14px', cursor: 'pointer' }
...
export default function TryMe() {
  ...
  return <div style={{ fontFamily: FONT, background: '#FAFAFB', minHeight: '100vh', color: '#111827', paddingBottom: 80 }}>
    <style>{TM_ANIM}</style>
    {/* header / progress */}
    ...
    <div style={{ position: 'relative', zIndex: 1, maxWidth: 1080, margin: '0 auto', padding: '24px 24px 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, margin: '20px 0 8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => setEst(null)} style={{ background: 'none', border: 'none', color: '#9CA3AF', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Start over</button>
...
function Coachmark({ tStep, depKey, onPrimary, onBack, canBack, primaryLabel, dim = true }) {
  ...
  return <div style={{ position: 'fixed', inset: 0, zIndex: 70, pointerEvents: 'none' }}>
    ...
    {/* caption bubble */}
    <div className="tmUp" style={{ position: 'absolute', top: bubble.top, left: bubble.left, width: W, maxWidth: '92vw', pointerEvents: 'auto', background: '#fff', borderRadius: 14, padding: 16, boxShadow: '0 18px 50px rgba(17,24,39,.30)', border: `1px solid ${PINK}26` }}>
      ...
      <div style={{ fontSize: 13.5, color: '#374151', lineHeight: 1.5, marginBottom: 13 }}>{tStep.text}</div>
      {tStep.wait
    ...
    <div data-tour="tm-rules" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {RULES.map((r) => <div key={r.key} style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: `1.5px solid ${PINK}33`, borderRadius: 12, padding: '14px 16px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#111827', marginBottom: 2 }}>{r.label}</div>
          <div style={{ fontSize: 12.5, color: '#6B7280', lineHeight: 1.45 }}>{r.tip}</div>
        </div>
...
function BuildingPanel() {
  return <div style={{ ...panel, textAlign: 'center', padding: '48px 24px', color: '#6B7280', fontSize: 14 }}>
    ...
    Building your rota… <span style={{ color: '#9CA3AF' }}>(the scheduler may take a few seconds to wake up)</span>
...
function TmRotaGrid({ staff, assignments, team }) {
  ...
  return <div style={{ ...panel, padding: '22px 24px' }}>
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16 }}>
      ...
      <span style={{ fontSize: 12.5, color: '#9CA3AF' }}>{team}</span>
    </div>
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760, tableLayout: 'fixed' }}>
        ...
        <thead><tr><th style={RTH_STAFF}></th>{DAYS.map((d) => <th key={d} style={RTH}><div style={{ fontWeight: 800, color: '#374151' }}>{d}</div></th>)}</tr></thead>
        <tbody>
          ...
          {staff.map((s) => { const blocks = assignments.filter((a) => String(a.staff_id) === String(s.id) || a.staff_name === s.name)
            return <tr key={s.id}>
              <td style={RTD_STAFF}><span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, fontWeight: 600, color: '#111827' }}><span style={{ width: 9, height: 9, borderRadius: 99, background: PINK, flexShrink: 0 }} />{s.name}{s.keyholder && <KeyMark size={12} />}</span></td>
