...
// ════════════════════════════════════════════════════════════════════════════
//  STAFF PAGE — UX SANDBOX v3 (mock data only)
//  Availability = ONE per-day control. Each day: on/off + (all-day OR a window).
//  Coverage is honest: a person fills a shift only if their window that day covers it.
// ════════════════════════════════════════════════════════════════════════════
...
const allDayAvail = () => Object.fromEntries(OPEN_DAYS.map((d) => [d, true]))

// avail: { [dayIndex]: true (all day) | [start,end] (window) | absent (not available) }
const INITIAL = [
  { id: 'a1', team: 'foh', name: 'Alex Rivera', role: 'Supervisor', contracted: 38, max: 45, wage: 13.5, keyholder: true, avail: { 0: true, 1: true, 2: true, 3: true, 4: true, 5: true } },
  { id: 'a2', team: 'foh', name: 'Sam Lee', role: 'Server', contracted: 16, max: 30, wage: 11.5, keyholder: false, avail: { 0: [13, 17], 1: [13, 17], 2: [13, 17], 3: [13, 17], 4: [13, 17] } },
  { id: 'a3', team: 'foh', name: 'Jordan Kim', role: 'Server', contracted: 20, max: 35, wage: 11.5, keyholder: false, avail: { 3: true, 4: true, 5: true } },
  { id: 'a4', team: 'kit', name: 'Chris Doyle', role: 'Chef', contracted: 40, max: 48, wage: 15, keyholder: false, avail: { 0: true, 1: true, 2: true, 3: true, 4: [9, 15] } },
  { id: 'a5', team: 'kit', name: 'Pat Owens', role: 'Kitchen Porter', contracted: 24, max: 40, wage: 11.44, keyholder: false, avail: { 0: true, 1: true, 2: true, 3: true, 4: true, 5: true } },
  { id: 'a6', team: 'mgmt', name: 'Robin Shah', role: 'Duty Manager', contracted: 40, max: 45, wage: 18, keyholder: true, avail: { 0: true, 1: true, 2: true, 3: true, 4: true } },
]
const SHIFTS = [
  { team: 'foh', name: 'Opener', start: 9, end: 13, days: [0, 1, 2, 3, 4, 5], staff: 2, keyholder: true },
  { team: 'foh', name: 'Closer', start: 13, end: 17, days: [0, 1, 2, 3, 4], staff: 2, keyholder: true },
  { team: 'kit', name: 'Prep', start: 9, end: 14, days: [0, 1, 2, 3, 4], staff: 1, keyholder: false },
  { team: 'kit', name: 'Service', start: 12, end: 17, days: [0, 1, 2, 3, 4, 5], staff: 2, keyholder: false },
  { team: 'mgmt', name: 'Duty Manager', start: 9, end: 17, days: [0, 1, 2, 3, 4], staff: 1, keyholder: true },
]
...
function dayLabel(days) {
  const p = activePreset(days)
  return p === 'all' ? 'Every open day' : p === 'weekdays' ? 'Weekdays' : p === 'weekend' ? 'Weekends' : days.map((d) => DAYS[d]).join(' ')
}
const initials = (name) => name.split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase()
const first = (name) => name.split(' ')[0]
const workDays = (s) => OPEN_DAYS.filter((d) => s.avail?.[d])
function windowForDay(s, d) { const a = s.avail?.[d]; if (!a) return null; return a === true ? BUSINESS[d] : a }
function canWork(s, d, sh) { const w = windowForDay(s, d); if (!w) return false; return w[0] <= sh.start + 0.001 && w[1] >= sh.end - 0.001 }
function completeness(s) {
  let n = 0
  if (s.name?.trim()) n++; if (s.role?.trim()) n++; if (s.wage > 0) n++; if (s.contracted > 0) n++; if (workDays(s).length) n++
  return n / 5
}
const openDayCount = (days) => days.filter((d) => OPEN_DAYS.includes(d)).length
function requiredHours(shifts) { return shifts.reduce((a, s) => a + (s.end - s.start) * s.staff * openDayCount(s.days), 0) }

function staffing(staff, shifts) {
  ...
  for (const sh of shifts) {
    for (const d of sh.days) {
      ...
      demand += sh.staff
      const q = staff.filter((st) => canWork(st, d, sh) && (!sh.keyholder || st.keyholder))
...
function suggestStaffFixGroup(teamStaff, g) {
  const sh = { start: g.start, end: g.end }, D = g.days
  if (g.kh) {
    const cand = teamStaff.find((st) => !st.keyholder && D.some((d) => canWork(st, d, sh)))
    if (cand) return { label: `Make ${first(cand.name)} a keyholder`, kind: 'keyholder', id: cand.id }
  }
  const addCand = teamStaff.find((st) => (!g.kh || st.keyholder) && workDays(st).some((d) => canWork(st, d, sh)) && D.some((d) => !st.avail?.[d]))
  ...
  // only widen someone whose WINDOWED day doesn't cover (never an all-day person — that'd be a no-op)
  const wideCand = teamStaff.find((st) => (!g.kh || st.keyholder) && D.some((d) => Array.isArray(st.avail?.[d]) && !canWork(st, d, sh)))
...
function readiness(staff, shifts) {
  const cov = staffing(staff, shifts)
  const req = requiredHours(shifts)
  const contracted = staff.reduce((a, s) => a + s.contracted, 0)
  const maxh = staff.reduce((a, s) => a + s.max, 0)
  // "can it be staffed at all" is judged against MAX hours (people can work into overtime)
  const capacityPct = req ? Math.min(100, Math.round((maxh / req) * 100)) : 100
  const coverableAtMax = maxh >= req
  const withinContract = contracted >= req
  return {
    coveragePct: cov.pct, overallPct: Math.min(cov.pct, capacityPct), short: cov.short,
    ready: cov.short.length === 0 && coverableAtMax, // rota CAN generate (overtime allowed)
...
function ClampBar({ value, color }) {
  ...
  return <div style={{ width: '100%', height: 10, borderRadius: 99, background: color + '18', overflow: 'hidden' }}><div style={{ width: `${Math.round(v * 100)}%`, height: '100%', background: value >= 1 ? '#16A34A' : color, borderRadius: 99, transition: 'width .3s' }} /></div>
}
...
function TimeRange({ start, end, onChange, accent = PINK, domain = [6, 22] }) {
...
// small chip button (grey by default, accent on hover) — clearly clickable, not a toggle
...
// ── ONE availability control: per-day on/off + (all-day OR window) ────────────────
function AvailabilityEditor({ s, patch, accent }) {
  const setDay = (d, val) => { const a = { ...(s.avail || {}) }; if (val === false) delete a[d]; else a[d] = val; patch({ avail: a }) }
  const allWeekOn = OPEN_DAYS.every((d) => s.avail?.[d] === true)
  return <div style={{ borderTop: '1px solid #F0F0F2', paddingTop: 16 }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
      <Label>Availability</Label>
      <div style={{ display: 'flex', gap: 8 }}>
        <MiniBtn active={allWeekOn} onClick={() => patch({ avail: allDayAvail() })} accent={accent}>All week</MiniBtn>
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {OPEN_DAYS.map((d) => {
        ...
        const w = allDay ? BUSINESS[d] : (Array.isArray(a) ? a : BUSINESS[d])
        return <div key={d} style={{ padding: '10px 0', borderBottom: '1px solid #F4F4F6' }}>
            ...
            {on
              ? <div style={{ display: 'inline-flex', background: '#F1F1F4', borderRadius: 8, padding: 2, gap: 2 }}>
                  {[['all', 'All day'], ['set', 'Set hours']].map(([k, lbl]) => {
                    ...
                    return <button key={k} onClick={() => setDay(d, k === 'all' ? true : (Array.isArray(a) ? a : [9, 17]))} style={{ fontSize: 11, fontWeight: act ? 700 : 600, padding: '4px 10px', borderRadius: 6, border: 'none', cursor: 'pointer', background: act ? '#fff' : 'transparent', color: act ? accent : '#9CA3AF', boxShadow: act ? '0 1px 2px rgba(0,0,0,.08)' : 'none', transition: 'all .12s' }}>{lbl}</button>
                  })}
                </div>
          ...
          {on && !allDay && <div style={{ marginTop: 10, paddingLeft: 46 }}>
            ...
            <TimeRange start={w[0]} end={w[1]} onChange={(x, y) => setDay(d, [x, y])} accent={accent} />
          </div>}
        </div>
      })}
    </div>
...
const FieldLabel = ({ children }) => <div style={{ fontSize: 11, fontWeight: 700, color: '#9CA3AF', letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 8 }}>{children}</div>
function Inspector({ s, patch, onDelete, saveState, onSave, accent = PINK }) {
  ...
  return <>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div><FieldLabel>Hourly wage</FieldLabel>
        <div style={{ position: 'relative' }}>
          <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 14, fontWeight: 700, color: '#9CA3AF' }}>£</span>
          <input value={s.wage} onChange={(e) => patch({ wage: parseFloat(e.target.value) || 0 })} type="number" step="0.25" style={inputStyle({ paddingLeft: 24 })} />
        </div>
      </div>
      ...
      <AvailabilityEditor s={s} patch={patch} accent={accent} />
...
// ── staff card ─────────────────────────────────────────────────────────────────
function StaffCard({ s, selected, onClick, accent = PINK }) {
  const wd = workDays(s)
  const allSame = wd.length > 0 && wd.every((d) => s.avail[d] === true)
  const availTxt = wd.length === 0 ? 'no availability' : allSame ? `${dayLabel(wd)}, all day` : `${dayLabel(wd)}, varies`
  return <div onClick={onClick} style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', border: `1.5px solid ${selected ? accent : '#ECECEF'}`, boxShadow: selected ? `0 0 0 3px ${accent}18` : '0 1px 2px rgba(0,0,0,.04)', transition: 'border-color .15s, box-shadow .15s' }}>
    <Bar value={completeness(s)} height={3} color={accent} />
    <div style={{ padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 13 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12.5, color: '#6B7280', marginTop: 3 }}>{s.role} · {s.contracted}/{s.max}h · £{s.wage.toFixed(2)} · {availTxt}</div>
      </div>
...
function CapacityLine({ r, accent, teamId, onFix }) {
  ...
  return <>
    ...
    <div style={{ display: 'flex', height: 10, borderRadius: 99, overflow: 'hidden', background: '#EFEFF2' }}>
      <div style={{ width: `${solid * 100}%`, background: withinContract ? '#16A34A' : accent }} />
      <div style={{ width: `${ot * 100}%`, background: accent + '40' }} />
    </div>
    <div style={{ fontSize: 13, fontWeight: 700, marginTop: 11, color: coverableAtMax ? '#16A34A' : '#EF4444' }}>{coverableAtMax ? '✓ Shifts covered' : `Short ${shortAtMaxH}h even at max`}</div>
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginTop: 9 }}>
      {row('Contracted', `${contracted}h`)}
      {row('Max available', `${maxh}h`)}
      {row('Shifts need', `${req}h`)}
    </div>
...
function ShortfallList({ staff, shifts, teamId, onFix }) {
  const groups = groupShortfalls(staffing(staff, shifts).short)
  return <>
    ...
    {groups.length === 0 ? <div style={{ fontSize: 12.5, color: '#16A34A', fontWeight: 600 }}>✓ Every shift can be filled by available staff.</div> : (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {groups.map((g, i) => {
          const fix = suggestStaffFixGroup(staff, g)
          return <div key={i}>
...
function TeamGlance({ staff, shifts, teamName, teamId, accent, onFix }) {
  const r = readiness(staff, shifts)
  return <div>
    ...
    <ShortfallList staff={staff} shifts={shifts} teamId={teamId} onFix={onFix} />
  </div>
}
function AllTeams({ staff, shifts, onFix }) {
  const [open, setOpen] = useState(null)
  const everyTeamReady = TEAMS.every((t) => readiness(staff.filter((s) => s.team === t.id), shifts.filter((s) => s.team === t.id)).ready)
  return <div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {TEAMS.map((t) => {
        const ts = staff.filter((s) => s.team === t.id), tsh = shifts.filter((s) => s.team === t.id)
        const r = readiness(ts, tsh), isOpen = open === t.id
        const ng = groupShortfalls(r.short).length
        const notReadyBits = []
        if (!r.coverableAtMax) notReadyBits.push(`short ${r.shortAtMaxH}h even at max`)
        if (ng) notReadyBits.push(`${ng} coverage gap${ng === 1 ? '' : 's'}`)
        return <div key={t.id} style={{ background: '#fff', border: '1px solid #ECECEF', borderRadius: 14, padding: '16px 18px' }}>
          <div onClick={() => setOpen(isOpen ? null : t.id)} style={{ cursor: 'pointer' }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 600, marginTop: 8 }}>
              {!r.ready
                ? <span style={{ color: '#92660B' }}>{notReadyBits.join(' · ')}</span>
                : r.withinContract
                  ? <span style={{ color: '#16A34A' }}>✓ Fully staffed</span>
          ...
          {isOpen && <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #F0F0F2', display: 'flex', gap: 28, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 240 }}><ShortfallList staff={ts} shifts={tsh} teamId={t.id} onFix={onFix} /></div>
          </div>}
        </div>
      })}
    </div>
...
export default function StaffLab() {
  ...
  const teamStaff = useMemo(() => staff.filter((s) => s.team === teamId), [staff, teamId])
  ...
  const add = useCallback((tId) => {
    const id = 'new-' + Math.round(Math.random() * 1e6)
    setStaff((prev) => [...prev, { id, team: tId, name: 'New staff member', role: '', contracted: 0, max: 40, wage: 11.44, keyholder: false, avail: Object.fromEntries(WEEKDAYS.filter((d) => OPEN_DAYS.includes(d)).map((d) => [d, true])) }])
    setTeamId(tId); setSelectedId(id); setSaveState('dirty')
  }, [])
  const remove = useCallback((id) => { setStaff((prev) => prev.filter((s) => s.id !== id)); setSelectedId(null) }, [])
  const save = useCallback(() => { setSaveState('saved'); setTimeout(() => setSaveState((x) => (x === 'saved' ? 'clean' : x)), 1800) }, [])
  const applyFix = useCallback((tId, fix) => {
    if (fix.kind === 'keyholder') setStaff((p) => p.map((s) => (s.id === fix.id ? { ...s, keyholder: true } : s)))
    else if (fix.kind === 'adddays') setStaff((p) => p.map((s) => (s.id === fix.id ? { ...s, avail: { ...s.avail, ...Object.fromEntries(fix.days.map((d) => [d, [fix.start, fix.end]])) } } : s)))
    else if (fix.kind === 'widen') setStaff((p) => p.map((s) => {
      if (s.id !== fix.id) return s
      const a = { ...s.avail }
      for (const d of fix.days) { const cur = a[d]; if (Array.isArray(cur)) a[d] = [Math.min(cur[0], fix.start), Math.max(cur[1], fix.end)] }
      return { ...s, avail: a }
    }))
    else setStaff((p) => [...p, { id: 'new-' + Math.round(Math.random() * 1e6), team: tId, name: fix.kh ? 'New keyholder' : 'New team member', role: '', contracted: fix.contracted ?? 0, max: Math.max(40, fix.contracted ?? 0), wage: 11.44, keyholder: !!fix.kh, avail: allDayAvail() }])
  }, [])
  ...
  return <div style={{ fontFamily: FONT, background: '#FAFAFB', minHeight: '100vh', color: '#111827' }}>
    <div style={{ padding: '12px 28px', background: '#fff', borderBottom: '1px solid #ECECEF', fontSize: 13, color: '#9CA3AF' }}>Staff · UX sandbox · mock data, nothing saves</div>

    <div style={{ display: 'flex', justifyContent: 'center', padding: '16px 0' }}>
      <div style={{ display: 'inline-flex', background: '#F1F1F4', borderRadius: 11, padding: 4, gap: 2 }}>
        {tabs.map((t) => {
          const active = t.id === teamId
          const count = t.id === 'all' ? staff.length : staff.filter((s) => s.team === t.id).length
    ...
    {isAll ? (
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '4px 24px 40px' }}><AllTeams staff={staff} shifts={SHIFTS} onFix={applyFix} /></div>
    ) : (
      <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start', padding: '4px 24px 40px', maxWidth: 1240, margin: '0 auto' }}>
        <div style={{ ...panel, width: 340, flexShrink: 0, position: 'sticky', top: 16, minHeight: 460 }}>
          <Inspector s={selected} patch={(p) => patch(selected.id, p)} onDelete={() => remove(selected.id)} saveState={saveState} onSave={save} accent={accent} />
        </div>
        <div style={{ flex: 1, minWidth: 300 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#6B7280' }}>{teamStaff.length} in {TEAMS.find((t) => t.id === teamId).name}</span>
            <button onClick={() => add(teamId)} style={{ fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: '#fff', background: accent, border: 'none', borderRadius: 9, padding: '9px 16px', cursor: 'pointer' }}>+ Add staff</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {teamStaff.map((s) => <StaffCard key={s.id} s={s} selected={selectedId === s.id} onClick={() => setSelectedId(s.id)} accent={accent} />)}
        ...
        <div style={{ ...panel, width: 300, flexShrink: 0, position: 'sticky', top: 16 }}>
          <TeamGlance staff={teamStaff} shifts={SHIFTS.filter((s) => s.team === teamId)} teamName={TEAMS.find((t) => t.id === teamId).name} teamId={teamId} accent={accent} onFix={applyFix} />
        </div>
      </div>
    )}
  </div>
